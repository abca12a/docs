...
```http
POST https://dimilinks.com/v1/messages
...
```bash
curl "https://dimilinks.com/v1/messages" \
...
<Note>
  Anthropic SDK 的 `base_url` 指到根域名 `https://dimilinks.com` 即可；SDK 内部会自动拼接 `/v1/messages`。
</Note>
