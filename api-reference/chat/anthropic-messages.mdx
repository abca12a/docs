...
<Note>
  Anthropic Messages 与其它公开 API 统一使用 `api-direct.dimilinks.com`。完整请求地址是 `https://api-direct.dimilinks.com/v1/messages`，并共用同一把 Bearer 密钥。
</Note>
...
| 参数 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `model` | string | 是 | Claude 模型 ID，例如 `claude-opus-4-8`、`claude-sonnet-5`、`claude-haiku-4-5`。 |
