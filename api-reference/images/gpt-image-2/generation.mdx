...
`data[].url` 是图片结果地址，请按以下规则处理：

- `/p/img/...`：相对路径，展示或下载前补成 `https://lmage2.dimilinks.com`。该路径不需要 Bearer，URL 自带签名和过期时间。
- `https://lmage2.dimilinks.com/p/img/...`：完整图片链接，可直接请求或展示。

异步任务查询返回的图片结果位于任务响应的 `data.data[]`，不是 `result.data[]`。图片 URL 中的 `img_xxx` 是图片资源 ID，不是提交接口返回的公开任务 ID。
