---
title: "CC Switch / Cherry Studio 一键导入"
description: "在 DimiLinks 控制台直接把密钥推送到 CC Switch 或 Cherry Studio。"
...
2. 在目标密钥右侧点「导入到 CC Switch」或「导入到 Cherry Studio」。导入 CC Switch 前，可以先在「CC Switch 配置给」里选择要写入的工具（Claude Code / Codex / Gemini CLI），默认按密钥模型自动推荐。
3. 浏览器会弹出系统级跳转，由对应客户端接管 deep link，自动写入 base URL、密钥与模型。

控制台会按密钥的「模型限制」与分组自动推荐目标 provider：

所有请求都使用 `api-direct.dimilinks.com`。一键导入会根据目标客户端是否自动追加 API 路径，写入带 `/v1` 或不带 `/v1` 的正确格式。
...
| 其它（GPT / Grok / 不限模型） | OpenAI 风格 | `https://api-direct.dimilinks.com/v1` |
...
- 导入成功但在 CC Switch 里找不到：provider 只会出现在导入时所选工具的列表里（Claude Code / Codex / Gemini 各自独立），请切到对应工具页签查看，或回控制台重新选择目标后再导入。
- 给 Claude Code 使用时，请选 Claude 分组的密钥；通用分组的密钥无法调用 Claude 系列模型。
...
| `app` | `claude` / `codex` / `gemini`，默认按密钥模型族推荐，可在导入前手动切换 |
| `name` | 在 CC Switch 内显示的 provider 名称 |
| `apiKey` | DimiLinks Bearer 密钥 |
| `endpoint` | base URL（按目标工具取 `https://api-direct.dimilinks.com/v1` 或 `https://api-direct.dimilinks.com`，不带结尾斜杠） |
