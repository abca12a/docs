...
- 调用 `https://api-direct.dimilinks.com/v1/*`（OpenAI 兼容）的接口，错误使用 OpenAI 风格 `error` 包装。
...
- 同一段业务代码如果同时调用两种协议，建议在 HTTP 客户端里统一抽象「读 `error.message` 或 `error.error.message`」，避免每个调用方各写一遍兼容逻辑。
