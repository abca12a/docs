...
`minimax-h3`、`seedance2.0-A`、`seedance2.5-A` 走按秒计费：费用 = 每秒单价 × 秒数，分辨率决定每秒单价。模型在 `/api/pricing` 的返回里带 `billing_mode: "tiered_expr"` 与 `billing_expr`，表达式声明了各分辨率档位的每秒单价：

| 模型 | 480p | 720p | 1080p |
| --- | ---: | ---: | ---: |
| `minimax-h3` | ¥0.16/秒 | ¥0.24/秒 | ¥0.70/秒 |
| `seedance2.0-A` | ¥0.60/秒 | ¥1.00/秒 | ¥1.80/秒 |
| `seedance2.5-A` | ¥0.70/秒 | ¥1.30/秒 | 不支持 |
