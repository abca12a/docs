...
DimiLinks のモデル API は、すべて次のリクエスト URL を使用します。

```text
https://api-direct.dimilinks.com/v1
```

OpenAI 互換 API では、この値をそのまま base URL に指定できます。Anthropic 公式 SDK や Claude Code などのネイティブクライアントは `/v1/messages` を自動的に追加するため、これらの `base_url` には `https://api-direct.dimilinks.com/` を指定してください。これにより、誤った `/v1/v1/messages` へのリクエストを防げます。どちらの設定も同じドメインと Bearer キーを使用します。

## 推奨フロー

```text
POST https://api-direct.dimilinks.com/v1/chat/completions          # 対話（OpenAI 互換、プログラミング用途に推奨）
POST https://api-direct.dimilinks.com/v1/messages                  # 対話（Anthropic ネイティブプロトコル）
POST https://api-direct.dimilinks.com/v1/images/generations?async=true  # 画像：非同期送信
POST https://api-direct.dimilinks.com/v1/videos/generations             # 動画：非同期送信（デフォルトで非同期）
GET  https://api-direct.dimilinks.com/v1/tasks/{task_id}                # 画像 / 動画タスク共通の照会
```
