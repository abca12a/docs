...
`data[].url` は画像結果の URL です。次のルールで処理してください。

- `/p/img/...`：相対パスです。表示またはダウンロードする前に `https://lmage2.dimilinks.com` を補ってください。このパスには Bearer は不要で、URL 自体に署名と有効期限が含まれています。
- `https://lmage2.dimilinks.com/p/img/...`：完全な画像 URL です。そのままリクエストまたは表示できます。

非同期タスクの画像結果は、タスクレスポンスの `data.data[]` にあります。`result.data[]` ではありません。画像 URL 内の `img_xxx` は画像プールのリソース ID であり、送信時に返される公開タスク ID とは異なります。
