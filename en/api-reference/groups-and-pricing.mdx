...
## Video billing: two groups, two rules

Video capability is split into two groups with different billing. Confirm which group a model belongs to before calling it; see [Video groups and billing rules](/en/api-reference/videos/groups).

### The `seedance/h3` group: per-second billing (tiered_expr)

`minimax-h3`, `seedance 2.0`, and `seedance 2.5` use per-second billing: cost = per-second price × seconds, with the resolution determining the per-second price (`seedance/h3` group ratio = 1). In `/api/pricing`, these models carry `billing_mode: "tiered_expr"` and a `billing_expr` that declares the per-second price of each resolution tier:

| Model | 480p | 720p | 1080p |
| --- | ---: | ---: | ---: |
| `minimax-h3` | ¥0.16/s | ¥0.24/s | ¥0.70/s |
| `seedance 2.0` | ¥0.60/s | ¥1.00/s | ¥1.80/s |
| `seedance 2.5` | ¥0.70/s | ¥1.30/s | unsupported |

### The `doubao seedance` group: per-token billing

`doubao-seedance-2-0-260128`, `doubao-seedance-2-0-fast-260128`, `doubao-seedance-2-0-mini-260615`, and `doubao-seedance-2-5-260628` use per-token billing (`quota_type = 0`) and support **720p only**. The `doubao seedance` group ratio is `0.7`, i.e. 30% off the official baseline. Token usage depends on duration × resolution and follows the standard token formula.

## Per-resolution image billing

| Model | 1k | 2k | 4k |
| --- | ---: | ---: | ---: |
| `gpt-image-2` | ¥0.08/image | ¥0.15/image | ¥0.30/image |
| `nano-banana-pro` | ¥0.12/image | ¥0.18/image | ¥0.195/image |

`gpt-image-2.5-flare` / `gpt-image-2.5-sunburst` are billed by token ratio (input 37.5×, output 2×); the actual cost depends on the image size.
