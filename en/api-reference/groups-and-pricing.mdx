...
## Per-second video billing (tiered_expr)

`minimax-h3`, `seedance2.0-A`, and `seedance2.5-A` use per-second billing: cost = per-second price × seconds, with the resolution determining the per-second price. In `/api/pricing`, these models carry `billing_mode: "tiered_expr"` and a `billing_expr` that declares the per-second price of each resolution tier:

| Model | 480p | 720p | 1080p |
| --- | ---: | ---: | ---: |
| `minimax-h3` | ¥0.16/s | ¥0.24/s | ¥0.70/s |
| `seedance2.0-A` | ¥0.60/s | ¥1.00/s | ¥1.80/s |
| `seedance2.5-A` | ¥0.70/s | ¥1.30/s | unsupported |
