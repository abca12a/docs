...
`data[].url` is an image result URL. Handle it as follows:

- `/p/img/...`: a relative path. Prefix it with `https://lmage2.dimilinks.com` before displaying or downloading it. The path requires no Bearer token because the URL includes a signature and expiration time.
- `https://lmage2.dimilinks.com/p/img/...`: a complete image URL that can be requested or displayed directly.

For asynchronous tasks, image results are in `data.data[]` on the task response, not `result.data[]`. The `img_xxx` in an image URL is an image-pool resource ID; it is different from the public task ID returned by submission.
