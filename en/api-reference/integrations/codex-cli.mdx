...
2. CC Switch automatically adds a `Codex` provider with the base URL `https://api-direct.dimilinks.com/v1`.
...
Treat `GET https://api-direct.dimilinks.com/v1/models` as the authoritative model catalog.

## Troubleshooting

- **`401 Invalid token`:** Check whether shell quoting truncated `api_key` / `OPENAI_API_KEY`.
- **`404`:** This occurs when the base URL is set to `https://api-direct.dimilinks.com/` without `/v1`. The OpenAI-compatible endpoint requires `/v1`.
